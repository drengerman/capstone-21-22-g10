...
data = load('train_image.mat');
objectData = data.data_change;

objectData.imageFilename = fullfile(pwd, objectData.imageFilename);

rng(0);
shuffledIndices = randperm(height(objectData));
idx = floor(0.8 * length(shuffledIndices));
trainingDataTbl = objectData(shuffledIndices(1:idx), :);
testDataTbl = objectData(shuffledIndices(idx+1:end), :);
